...
const ShopBilling = () => {
  ...
  const getDetailsKey = useCallback(
    () => `shopDetails:${authUserId || "anon"}`,
    [authUserId]
  );
  const saveShopDetailsToLocal = (
    _routeId: string,
    name: string,
    address?: string,
    phone?: string
  ...
  const removeShopName = (_routeId: string, name: string) => {
    ...
    let updatedLocal: string[] = Array.isArray(existingLocal)
      ? existingLocal
      : [];
    ...
    const updatedHidden: string[] = Array.isArray(existingHidden)
      ? existingHidden
      : [];
  ...
  const getPcsPerBoxFromProduct = (product: unknown): number => {
    ...
    if (typeof p.pcs_per_box === "number" && p.pcs_per_box > 0)
      return p.pcs_per_box;
    const box = (p.box_price ?? p.price) as number | undefined;
    const pcs = (p.pcs_price ?? (box ? box / 24 : undefined)) as
      | number
      | undefined;
  ...
  const fetchProductsAndStock = useCallback(
    async (route: string, date: string) => {
      try {
        ...
        const saleItemsWithStock = productsData.flatMap((product) => {
          ...
          if (stockData?.stock) {
            ...
            const boxStock = items.find(
              (s) => s.productId === product.id && s.unit === "box"
            );
            const pcsStock = items.find(
              (s) =>
          ...
          return [
            {
              ...
              unit: "box" as const,
              quantity: 0,
              price: product.box_price ?? product.price,
              total: 0,
            ...
            },
            {
              ...
              quantity: 0,
              price:
                product.pcs_price ?? (product.box_price ?? product.price) / ppb,
              total: 0,
  ...
  const getPcsPerBox = (productId: string): number => {
    ...
    if (!p) return 24;
    if (typeof p.pcs_per_box === "number" && p.pcs_per_box > 0)
      return p.pcs_per_box as number;
  ...
  const updateQuantity = (
    ...
    setSaleItems((prev) => {
      const boxItem = prev.find(
        (i) => i.productId === productId && i.unit === "box"
      );
      const pcsItem = prev.find(
        (i) => i.productId === productId && i.unit === "pcs"
      );
        ...
        return prev.map((item) =>
          item.productId === productId && item.unit === "box"
            ? {
                ...
                quantity: Math.max(
                  0,
                  Math.min(maxAllowed, item.quantity + change)
                ),
                total:
                  Math.max(0, Math.min(maxAllowed, item.quantity + change)) *
                  item.price,
              }
            : item
        );
      ...
      if (unit === "box") {
        ...
        return prev.map((item) =>
          item.productId === productId && item.unit === "box"
            ? {
                ...item,
                quantity: Math.max(0, Math.min(maxAllowed, quantity)),
                total: Math.max(0, Math.min(maxAllowed, quantity)) * item.price,
              }
            : item
        );
      } else {
    ...
    setSaleItems((prev) =>
      prev.map((item) => {
        if (item.productId === productId && item.unit === unit) {
          const validPrice =
            Number.isFinite(newPrice) && newPrice >= 0 ? newPrice : 0;
          return {
            ...item,
            price: validPrice,
  ...
  const adjustTempQuantity = (delta: number) => {
    setTempQuantity((q) => {
      ...
      if (selectedProduct) {
        const boxItem = saleItems.find(
          (i) => i.productId === selectedProduct.id && i.unit === "box"
        );
        const pcsItem = saleItems.find(
          (i) => i.productId === selectedProduct.id && i.unit === "pcs"
        );
        const boxAvail = boxItem?.availableStock ?? 0;
        const pcsAvail = pcsItem?.availableStock ?? 0;
        const max =
  ...
  const handleAddProductToSale = () => {
    ...
    const boxItem = saleItems.find(
      (i) => i.productId === pid && i.unit === "box"
    );
    const pcsItem = saleItems.find(
      (i) => i.productId === pid && i.unit === "pcs"
    );
    const boxAvail = boxItem?.availableStock ?? 0;
    const pcsAvail = pcsItem?.availableStock ?? 0;
    const maxAvail =
      unitMode === "pcs" ? pcsAvail + boxAvail * getPcsPerBox(pid) : boxAvail;
  ...
  const isValidForBilling = () => {
    ...
    return (
      sold.length > 0 &&
      sold.every((item) => item.quantity > 0 && item.price >= 0)
    );
  };
  ...
  const handlePrintBill = async () => {
    ...
    try {
      ...
      if (currentRoute && currentDate) {
        ...
        setSaleItems((prev) =>
          prev.map((i) => ({ ...i, quantity: 0, total: 0 }))
        );
      }
      setLoading(false);
          ...
          <Card className="border-0 shadow-strong">
            ...
            <CardContent className="px-4 sm:px-6">
              <div className="space-y-6 sm:space-y-8">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (value.trim().length >= 1) {
                          ...
                          const exact = shopSuggestions.find(
                            (n) =>
                              n.toLowerCase() === value.trim().toLowerCase()
                          );
                          if (exact && currentRoute) {
                            const details = getShopDetailsFromLocal(
                              currentRoute,
                              exact
                            );
                            if (details) {
                              if (typeof details.address === "string")
                                setShopAddress(details.address);
                              if (typeof details.phone === "string")
                                setShopPhone(details.phone);
                            }
                          }
                        } else {
                      ...
                      onFocus={() => {
                        if (shopName.trim().length >= 1) {
                          const match = shopSuggestions
                            .filter((n) =>
                              n.toLowerCase().startsWith(shopName.toLowerCase())
                            )
                            .slice(0, 8);
                          setFilteredSuggestions(match);
                          setShowSuggestions(match.length > 0);
                ...
                <div className="space-y-2">
                  <Label className="text-sm sm:text-base font-semibold flex items-center gap-2">
                    <MapPin className="w-4 h-4" /> Address / Village
                  </Label>
                  <Input
                    type="text"
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-base sm:text-lg font-semibold flex items-center gap-2">
                      <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5" /> Select
                      Products
                    </Label>
                    <div className="flex items-center gap-2">
                  ...
                  >
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Search and quickly add a product with unit mode.
                        </DialogDescription>
                      </DialogHeader>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              const matchList = products
                                .filter((p) =>
                                  nameMatchesQueryByWordPrefix(p.name, q)
                                )
                                .filter((p) => {
                                  const boxItem = saleItems.find(
                                    (s) =>
                                      s.productId === p.id && s.unit === "box"
                                  );
                                  const pcsItem = saleItems.find(
                                    (s) =>
                                      s.productId === p.id && s.unit === "pcs"
                                  );
                                  const totalAvail =
                                    (boxItem?.availableStock || 0) +
                                    (pcsItem?.availableStock || 0);
                                  return totalAvail > 0;
                                });
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                      Avail: {boxAvail} Box, {pcsAvail} pcs
                                    </span>
                                  </button>
                                );
                              })}
                                ...
                                onChange={(e) => {
                                  ...
                                  const max =
                                    unitMode === "pcs"
                                      ? pcsAvail +
                                        boxAvail *
                                          getPcsPerBox(selectedProduct.id)
                                      : boxAvail;
                                  setTempQuantity(
                      ...
                      <DialogFooter className="sm:justify-end">
                        ...
                        <Button
                          ...
                          disabled={!selectedProduct || tempQuantity <= 0}
                        >
                          Add
                        </Button>
                      </DialogFooter>
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <Button
                                    ...
                                    size="icon"
                                    onClick={() =>
                                      updateQuantity(i.productId, i.unit, -1)
                                    }
                                    className="h-8 w-8"
                                  >
                                    <Minus className="w-4 h-4" />
                  ...
                  <div className="grid gap-3 sm:gap-4">
                    {products
                      .map((prod) => ({
                        ...
                        avail:
                          (saleItems.find(
                            (s) => s.productId === prod.id && s.unit === "box"
                          )?.availableStock || 0) +
                          (saleItems.find(
                            (s) => s.productId === prod.id && s.unit === "pcs"
                          )?.availableStock || 0),
                      }))
                      ...
                      .map((product) => {
                        ...
                        const pcsItem = saleItems.find(
                          (s) => s.productId === product.id && s.unit === "pcs"
                        );
                        const boxAvail = boxItem?.availableStock || 0;
                        const pcsAvail = pcsItem?.availableStock || 0;
                        ...
                        const boxPrice =
                          boxItem?.price ?? product.box_price ?? product.price;
                        const pcsPrice =
                          pcsItem?.price ??
                          product.pcs_price ??
                          (product.box_price ?? product.price ?? 0) / 24;
                        const availableStock = boxAvail + pcsAvail;
                        const lineTotal =
                          (boxItem?.total || 0) + (pcsItem?.total || 0);
                        return (
                          <Card
                            key={product.id}
                            className={`border transition-colors active:border-primary ${
                              availableStock === 0
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <Button
                                          ...
                                          size="icon"
                                          onClick={() =>
                                            updateQuantity(product.id, "box", 1)
                                          }
                                          disabled={
                                            boxQty >= boxAvail || boxAvail === 0
                                          }
                                          className="h-9 w-9"
                                        >
                                          <Plus className="w-4 h-4" />
                ...
                <div className="sticky bottom-3 sm:static bg-background/95 backdrop-blur-sm sm:bg-transparent sm:backdrop-blur-none p-1 sm:p-0 -mx-2 sm:mx-0 rounded-md sm:rounded-none">
                  ...
                  >
                    <Check className="w-5 h-5 mr-2" /> Generate Bill
                  </Button>
                </div>
              </div>
            </CardContent>
          ...
          <div className="receipt-58mm">
            ...
            <div style={{ textAlign: "center", marginBottom: "4px" }}>
              ...
              <div style={{ marginTop: "2px", fontSize: "8px" }}>
                ...
                {(printSnapshot?.routeName || currentRouteName) && (
                  <p style={{ margin: "0", fontWeight: "bold" }}>
                    Route: {printSnapshot?.routeName || currentRouteName}
                  </p>
                )}
              </div>
            </div>
            ...
            <div style={{ marginBottom: "4px" }}>
              ...
              >
                ...
                <tbody>
                  {(printSnapshot?.items || getSoldItems()).map(
                    (item, index) => (
            ...
            >
              ...
              >
                ...
                <span style={{ fontSize: "12px", fontWeight: "bold" }}>
                  ₹{(printSnapshot?.total ?? calculateTotal()).toFixed(2)}
                </span>
              </div>
              <div style={{ fontSize: "8px", textAlign: "right" }}>
                Items:{" "}
                {(printSnapshot?.items || getSoldItems()).reduce(
