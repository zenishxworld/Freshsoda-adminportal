...
const BillHistory = () => {
  ...
  const getRouteName = (routeId: string) =>
    routesMap[routeId] || "Unknown Route";
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground hidden xs:block">
                    View all bills created for a day
                  </p>
                </div>
              </div>
            </div>
        ...
        <Card className="border-0 shadow-strong">
          <CardHeader className="pb-3 sm:pb-4 px-4 sm:px-6">
            ...
            <CardDescription className="text-sm sm:text-base">
              Choose date to see all bills (works for every route)
            </CardDescription>
          </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    <Select
                      value={filterRouteId}
                      onValueChange={(v) => setFilterRouteId(v)}
                    >
                      <SelectTrigger
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div className="space-y-1">
                              <div className="text-sm text-muted-foreground">
                                {sale.date} · {formatTime(sale.created_at)}
                              </div>
                              <div className="text-base font-semibold text-foreground">
                                {sale.shop_name}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                {getRouteName(sale.route_id)}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                Items: {totalItems}
                              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between mb-3 print:hidden">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {getRouteName(selectedSale.route_id)}
                    </div>
                  </div>
                  ...
                  <Card className="border-0 shadow-strong print:shadow-none print:border-0">
                    <CardContent className="p-6 sm:p-8 print:p-2">
                      ...
                      <div className="text-center mb-6 print:mb-3">
                        <h1 className="text-2xl sm:text-3xl font-bold text-foreground print:text-lg">
                          BHAVYA ENTERPRICE
                        </h1>
                        ...
                        <div className="mt-2 text-xs text-foreground print:text-[10px]">
                          ...
                          <p className="font-semibold text-primary">
                            Route: {getRouteName(selectedSale.route_id)}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mb-6 print:mb-3 pb-3 border-t border-b border-dashed">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="font-semibold text-foreground print:text-[11px]">
                            Shop:
                          </span>
                        </div>
                        <p className="text-lg font-bold text-foreground pl-7 print:text-sm">
                          {selectedSale.shop_name}
                        </p>
                        {!Array.isArray(selectedSale.products_sold) &&
                          selectedSale.products_sold?.shop_address && (
                            <p className="text-xs text-muted-foreground pl-7 mt-1 print:text-[10px]">
                              Address/Village:{" "}
                              {selectedSale.products_sold.shop_address}
                            </p>
                          )}
                        {!Array.isArray(selectedSale.products_sold) &&
                          selectedSale.products_sold?.shop_phone && (
                            <p className="text-xs text-muted-foreground pl-7 print:text-[10px]">
                              Phone: {selectedSale.products_sold.shop_phone}
                            </p>
                          )}
                      </div>
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="border-b border-foreground border-dashed">
                              <th className="text-left py-2 font-bold text-foreground print:text-[11px] print:py-1">
                                Item
                              </th>
                              <th className="text-center py-2 font-bold text-foreground print:text-[11px] print:py-1">
                                Qty
                              </th>
                              <th className="text-right py-2 font-bold text-foreground print:text-[11px] print:py-1">
                                Rate
                              </th>
                              <th className="text-right py-2 font-bold text-foreground print:text-[11px] print:py-1">
                                Amt
                          ...
                          <tbody>
                            {(Array.isArray(selectedSale.products_sold)
                              ? selectedSale.products_sold
                              : selectedSale.products_sold?.items || []
                            ).map((item: any, index: number) => (
                              <tr
                                key={index}
                                className="border-b border-border"
                              >
                                <td className="py-3 text-foreground print:text-[11px] print:py-1">
                                  {item.productName || item.name}
                                </td>
                                <td className="py-3 text-center text-foreground print:text-[11px] print:py-1">
                                  {item.quantity}{" "}
                                  {item.unit === "box"
                                    ? "Box"
                                    : item.unit === "pcs"
                                ...
                                <td className="py-3 text-right text-foreground print:text-[11px] print:py-1">
                                  ₹{(item.price ?? 0).toFixed(2)}
                                </td>
                                <td className="py-3 text-right font-semibold text-foreground print:text-[11px] print:py-1">
                                  ₹
                                  {(
                                    item.total ??
                      ...
                      <div className="border-t border-foreground border-dashed pt-3 print:pt-2">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-lg font-bold text-foreground print:text-sm">
                            TOTAL:
                          </span>
                          <span className="text-2xl font-bold text-primary print:text-base">
                      ...
                      <div className="mt-6 pt-3 border-t border-dashed text-center print:mt-4 print:pt-2">
                        ...
                        <p className="text-xs text-muted-foreground mt-1 print:text-[9px]">
                          Have a great day!
                        </p>
                      </div>
                    </CardContent>
                  ...
                  <div id="print-receipt-container" style={{ display: "none" }}>
                    <div className="receipt-58mm">
                      ...
                      >
                        ...
                        >
                          Shop: {selectedSale?.shop_name}
                        </p>
                        {!Array.isArray(selectedSale?.products_sold) &&
                          (selectedSale as any)?.products_sold
                            ?.shop_address && (
                            <p style={{ fontSize: "8px", margin: 0 }}>
                              Addr:{" "}
                              {(selectedSale as any).products_sold.shop_address}
                            </p>
                          )}
                        {!Array.isArray(selectedSale?.products_sold) &&
                          (selectedSale as any)?.products_sold?.shop_phone && (
                            <p style={{ fontSize: "8px", margin: 0 }}>
                              Ph:{" "}
                              {(selectedSale as any).products_sold.shop_phone}
                            </p>
                        ...
                        >
                          ...
                          <tbody>
                            {(Array.isArray(selectedSale.products_sold)
                              ...
                              <tr key={index}>
                                <td
                                  style={{ padding: "1px 0", fontSize: "8px" }}
                                >
                                  {item.productName || item.name}
                                </td>
                                <td
